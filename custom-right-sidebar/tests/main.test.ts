...
import CustomRightSidebarPlugin from "../src/main";
import { DEFAULT_SETTINGS } from "../src/types/settings";

const VIEW_TYPE_CUSTOM_RIGHT_SIDEBAR = "custom-right-sidebar-view";

// Mock the CustomRightSidebarView
vi.mock("../src/components/custom-right-sidebar-view", () => ({
	CustomRightSidebarView: vi.fn().mockImplementation(() => ({
		updateContent: vi.fn(),
	})),
}));
...
describe("CustomRightSidebarPlugin", () => {
	let plugin: CustomRightSidebarPlugin;
	let mockApp: any;
	...
	beforeEach(async () => {
		...
		plugin = new CustomRightSidebarPlugin(mockApp, {} as any);
	...
	describe("view management", () => {
		it("should create view with settings getter function", async () => {
			...
			expect(CustomRightSidebarView).toHaveBeenCalledWith(mockLeaf, expect.any(Function));
		});
		...
		it("should provide current settings to view", async () => {
			...
			// Check that CustomRightSidebarView was called with the correct arguments
			expect(CustomRightSidebarView).toHaveBeenCalledWith(mockLeaf, expect.any(Function));

			// Get the settings getter function from the constructor call
			const settingsGetter = vi.mocked(CustomRightSidebarView).mock.calls[0][1];
